...
class SovereignSentience:
    ...
    async def get_htf_context(self):
        try:
            # tld='com' ile Binance kısıtlamasını aşmayı deniyoruz
            ...
            logging.info("🏛️ HTF Sync Basarili")
        except Exception as e:
            logging.error(f"Veri Hatasi: {e}")
    ...
    async def handle_trades(self):
        ...
        async with bm.aggtrade_socket(SYMBOL) as stream:
            while True:
                ...
                self.cvd += -float(msg['q']) if msg['m'] else float(msg['q'])

    async def start(self):
        # AsyncClient başlatma yöntemi güncellendi
        self.client = await AsyncClient.create(tld='com')
        await self.get_htf_context()
        telegram_send("🛰️ Sovereign Sentience Baslatildi!")
        await asyncio.gather(self.handle_trades(), self.handle_klines())

async def main():
    # Flask sunucusunu arka planda başlat
    threading.Thread(target=run_web, daemon=True).start()
    bot = SovereignSentience()
    await bot.start()

if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    try:
        # Runtime Error hatasını çözen modern çalıştırma yöntemi
        asyncio.run(main())
